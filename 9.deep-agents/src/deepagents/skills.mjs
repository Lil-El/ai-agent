...
// 创建 LocalShellBackend 实例，可以执行 shell 命令
...
const agent = createAgent({
  ...
  middleware: [
    createSkillsMiddleware({
      backend,
      sources: [skills], // sources 指定 skills 目录
    }),
...
const stream = await agent.streamEvents({ messages: [new HumanMessage(prompt)] }, { recursionLimit: 100 });
...
try {
  for await (const event of stream) {
    ...
    if (event.event === "on_tool_start") {
      ...
      process.stdout.write(`\n\n→ ${name}：${JSON.stringify(event.data.input)} \n\n`);
    }
