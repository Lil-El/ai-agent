...
class Mem0MemoryStore {
  ...
  /**
   *
   * @param {{client: MemoryClient}} param0
   */
...
async function invokeWithMemory(agent, redisStore, mem0Store, sessionId, userText) {
  ...
  // 先放长期记忆，然后短期记忆，然后是用户问题
  const invokeMessages = [...(memoryMsg ? [memoryMsg] : []), ...history, new HumanMessage(userText)];
...
const llmOpts = {
  ...
  configuration: { baseURL: process.env.OLLAMA_BASE_URL },
  temperature: 0,
  think: false,
};

const model = new ChatOllama(llmOpts);

const classifier = new ChatOllama(llmOpts).withStructuredOutput(memorySchema);
...
try {
  while (true) {
    const userText = (await rl.question("用户: ")).trim();
 ...
 *
 * 二、自我介绍
 * 我叫小明，住在杭州，平时喜欢骑行和摄影。我对海鲜过敏，出差尽量别安排沿海城市。
 * → 换天聊还得知道的事，写 user 层。
 *
 * 三、这会儿在干嘛
 * 这次我们先写 Q1 季度总结，大纲分三块：项目复盘、数据指标、下季度计划。项目复盘里重点写 order-service 的 500 错误排查过程。
 * → 只管这次聊天的事，写 session 层。
 *
 * 四、长期背景 + 手头活
 * 我长期做后端开发，这次会话的任务是排查 payment-api 超时，先从 P99 日志看起。另外我之后技术回答都希望带代码示例，这个一直记住。
