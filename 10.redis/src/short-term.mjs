...
async function invokeWithMemory(agent, store, sessionId, userText) {
  ...
  const result = await agent.invoke({ messages: [...history, new HumanMessage(userText)] }, { recursionLimit: 20 });

  await store.saveMessages(sessionId, result.messages);
  console.log(`  ↳ 已保存 ${result.messages.length} 条消息到 Redis，TTL ${await store.ttl(sessionId)} 秒`);
...
redis.on("connect", () => console.log("✅ 已连接到 Redis"));
...
try {
  while (true) {
    const userText = await rl.question("你: ");
    ...
    console.log("助手:", messages[messages.length - 1].text);
    console.log("当前消息数：", messages.length, "条");
